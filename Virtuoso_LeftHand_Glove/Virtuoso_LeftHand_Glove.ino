...
int fing3 = A2;
int fing4 = A3;
float POWER = 3.52; // Voltage used to power Arduino

// Schmitt trigger voltage values for each finger
int v11=1024*1.6/POWER; //pointer
int v12=1024*1.25/POWER;  //pointer

int v21=1024*1.8/POWER;  //middle
int v22=1024*1.35/POWER;  //middle

int v31=1024*1.75/POWER;  //ring
int v32=1024*1.5/POWER;   //ring

int v41=1024*1.75/POWER;   //pinky
int v42=1024*1.65/POWER;   //pinky
...
float threshRot = 400; // Threshold value for rotation
...
/// Gyro Stuff ///   
  if(rotWait == 0){
    if(abs(roll) > abs(pitch) && abs(roll) > abs(yaw)){
      if(abs(roll) > threshRot){
        Serial.println(50);
        rotWait = rotWaitVal;
      }
    }
    else if(abs(pitch) > abs(yaw) && abs(pitch) > abs(roll)){ 
